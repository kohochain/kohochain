...
@Slf4j(topic = "DB")
@Component
public class DynamicPropertiesStore extends khcStoreWithRevoking<BytesCapsule> {
  ...
  private static final byte[] MAX_LOCKED_TIME = "MAX_LOCKED_TIME".getBytes();

  private static final byte[] MIN_LOCKED_TIME = "MIN_LOCKED_TIME".getBytes();
  ...
  private static final byte[] CREATE_SMART_CONTRACT_FEE = "CREATE_SMART_CONTRACT_FEE".getBytes();
  ...
  private DynamicPropertiesStore(@Value("properties") String dbName) {
    ...
    try {
      ...
      this.saveMaxFrozenTime(0);
    }

    try {
      this.getMinFrozenTime();
    } catch (IllegalArgumentException e) {
      this.saveMinFrozenTime(0);
    }

    try {
      this.getMaxLockedTime();
    } catch (IllegalArgumentException e) {
      this.saveMaxLockedTime(3);
    }

    try {
      this.getMinLockedTime();
    } catch (IllegalArgumentException e) {
      this.saveMinLockedTime(3);
    }
    ...
    try {
      ...
      this.saveWitnessPayPerBlock(0L);  //出块奖励 0
    }

    try {
      this.getWitnessStandbyAllowance();
    } catch (IllegalArgumentException e) {
      this.saveWitnessStandbyAllowance(0L); //超级节点奖励为0
    }
    ...
    try {
      ...
      this.saveAccountUpgradeCost(50_000_000_00L); //节点竞选cost 50fortest
    }
    ...
    try {
      ...
      this.saveTotalNetLimit(0L);                //全网总的带宽为0
    }
    ...
    try {
      ...
      this.saveTotalEnergyLimit(0L);                    //全网总的能量为0
    }

    try {
      this.getEnergyFee();
    } catch (IllegalArgumentException e) {
      this.saveEnergyFee(1L);// 10 DATA per energy
    }
    ...
    try {
      ...
      this.saveCreateAccountFee(10000L); // 创建账号消耗10000 MA
    }
    ...
    try {
      ...
      this.saveTransactionFee(1L); // 100DATA/byte
    }


    try {
      this.getCreateSmartContractFee();
    } catch (IllegalArgumentException e) {
      this.saveCreateSmartContractFee(1000_000_000L); // 部署合约消耗10
    }

    try {
      this.getAssetIssueFee();
    } catch (IllegalArgumentException e) {
      this.saveAssetIssueFee(100_000_000L);    //发币消耗 10
    }

    try {
      this.getUpdateAccountPermissionFee();
    } catch (IllegalArgumentException e) {
      this.saveUpdateAccountPermissionFee(100_000);  //更新账户权限0.001
    }

    try {
      this.getMultiSignFee();
    } catch (IllegalArgumentException e) {
      this.saveMultiSignFee(10000L);
    }

    try {
      this.getExchangeCreateFee();
    } catch (IllegalArgumentException e) {
      this.saveExchangeCreateFee(0L);
    }
    ...
    try {
      ...
      String contractType = "7fff1fc0033e0200000000000000000000000000000000000000000000000000";
  ...
  public void saveMaxLockedTime(int maxLockedTime) {
    logger.debug("MAX_LOCKED_NUMBER:" + maxLockedTime);
    this.put(MAX_LOCKED_TIME,
            new BytesCapsule(ByteArray.fromInt(maxLockedTime)));
  }

  public int getMaxLockedTime() {
    return Optional.ofNullable(getUnchecked(MAX_LOCKED_TIME))
            .map(BytesCapsule::getData)
            .map(ByteArray::toInt)
            .orElseThrow(
                    () -> new IllegalArgumentException("not found MAX_LOCKED_TIME"));
  }
  ...
  public void saveMinLockedTime(int minLockedTime) {
    logger.debug("MIN_LOCKED_NUMBER:" + minLockedTime);
    this.put(MIN_LOCKED_TIME,
            new BytesCapsule(ByteArray.fromInt(minLockedTime)));
  }
  ...
  public int getMinLockedTime() {
    return Optional.ofNullable(getUnchecked(MIN_LOCKED_TIME))
            .map(BytesCapsule::getData)
            .map(ByteArray::toInt)
            .orElseThrow(
                    () -> new IllegalArgumentException("not found MIN_LOCKED_TIME"));
  }
  ...
  public long getCreateSmartContractFee() {
    return Optional.ofNullable(getUnchecked(CREATE_SMART_CONTRACT_FEE))
            .map(BytesCapsule::getData)
            .map(ByteArray::toLong)
            .orElseThrow(
                    () -> new IllegalArgumentException("not found CREATE_SMART_CONTRACT_FEE"));
  }


  public void saveCreateSmartContractFee(long fee) {
    this.put(CREATE_SMART_CONTRACT_FEE,
            new BytesCapsule(ByteArray.fromLong(fee)));
  }
  ...
  public void saveTotalStoragePool(long kht) {
    this.put(TOTAL_STORAGE_POOL,
        new BytesCapsule(ByteArray.fromLong(kht)));
  }
  ...
  public void saveTotalStorageTax(long kht) {
  ...
  public int getBlockFilledSlotsNumber() {
    return Parameter.ChainConstant.BLOCK_FILLED_SLOTS_NUMBER;
  }
