...
public class StorageMarket {
  ...
  private long exchange_to_supply(boolean iskht, long quant) {
    logger.info("iskht: " + iskht);
    long balance = iskht ? dbManager.getDynamicPropertiesStore().getTotalStoragePool() :
...
//    }
  ...
  private long exchange_to_supply2(boolean iskht, long quant) {
    logger.info("iskht: " + iskht);
    long balance = iskht ? dbManager.getDynamicPropertiesStore().getTotalStoragePool() :
        dbManager.getDynamicPropertiesStore().getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean iskht, long supplyQuant) {
    long balance = iskht ? dbManager.getDynamicPropertiesStore().getTotalStoragePool() :
        dbManager.getDynamicPropertiesStore().getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean iskht) {
    long relay = exchange_to_supply(iskht, from);
    return exchange_from_supply(!iskht, relay);
  }
  ...
  public long calculateTax(long duration, long limit) {
    ...
    double millisecondPerYear = (double) Parameter.ChainConstant.MS_PER_YEAR;
