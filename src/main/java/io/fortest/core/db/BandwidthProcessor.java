...
public class BandwidthProcessor extends ResourceProcessor {
  ...
  @Override
  public void consume(TransactionCapsule kht, TransactionTrace trace)
          throws ContractValidateException, AccountResourceInsufficientException, TooBigTransactionResultException {
    List<Contract> contracts = kht.getInstance().getRawData().getContractList();
    if (kht.getResultSerializedSize() > Constant.MAX_RESULT_SIZE_IN_TX * contracts.size()) {
    ...
    if (dbManager.getDynamicPropertiesStore().supportVM()) {
      bytesSize = kht.getInstance().toBuilder().clearRet().build().getSerializedSize();
    } else {
      bytesSize = kht.getSerializedSize();
    }
    ...
    for (Contract contract : contracts) {
      ...
      logger.debug("khtId {},bandwidth cost :{}", kht.getTransactionId(), bytesSize);
