...
public class WitnessController {
  ...
  public long getSlotAtTime(long when) {
    ...
    return (when - firstSlotTime) / Parameter.ChainConstant.BLOCK_PRODUCED_INTERVAL + 1;
  }
  ...
  public long getAbSlotAtTime(long when) {
    return (when - getGenesisBlock().getTimeStamp()) / Parameter.ChainConstant.BLOCK_PRODUCED_INTERVAL;
  }
  ...
  public ByteString getScheduledWitness(final long slot) {
    ...
    int singleRepeat = Parameter.ChainConstant.SINGLE_REPEAT;
...
//          account.createReadableString());
  ...
  public void updateWitness() {
    ...
    if (countWitness.isEmpty()) {
      ...
      if (newWitnessAddressList.size() > Args.getInstance().getMaxActiveWitnessNum()) {
        setActiveWitnesses(newWitnessAddressList.subList(0, Args.getInstance().getMaxActiveWitnessNum()));
      } else {
        setActiveWitnesses(newWitnessAddressList);
      }

      if (newWitnessAddressList.size() > Parameter.ChainConstant.WITNESS_STANDBY_LENGTH) {
        payStandbyWitness(newWitnessAddressList.subList(0, Parameter.ChainConstant.WITNESS_STANDBY_LENGTH));
      } else {
