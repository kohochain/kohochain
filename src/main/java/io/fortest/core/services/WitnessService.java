...
import io.fortest.core.witness.BlockProductionCondition;
...
import io.fortest.common.backup.BackupManager.BackupStatusEnum;
import io.fortest.common.backup.BackupServer;
...
public class WitnessService implements Service {
  ...
  private khcApplicationContext context;
  ...
  private long blockCycle =
      Parameter.ChainConstant.BLOCK_PRODUCED_INTERVAL * Args.getInstance().getMaxActiveWitnessNum();
  private Cache<ByteString, Long> blocks = CacheBuilder.newBuilder().maximumSize(10).build();

  /**
   * Construction method.
   */
  public WitnessService(Application khcApp, khcApplicationContext context) {
    this.khcApp = khcApp;
    this.context = context;
    backupManager = context.getBean(BackupManager.class);
    backupServer = context.getBean(BackupServer.class);
    khcNetService = context.getBean(khcNetService.class);
    generateThread = new Thread(scheduleProductionLoop);
    manager = khcApp.getDbManager();
  ...
  private BlockProductionCondition tryProduceBlock() throws InterruptedException {
    ...
    if (this.needSyncCheck) {
      ...
      if (nexSlotTime > now) { // check sync during first loop
        ...
        logger.debug("Not sync ,now:{},headBlockTime:{},headBlockNumber:{},headBlockId:{}",
            new DateTime(now),
            new DateTime(this.khcApp.getDbManager().getDynamicPropertiesStore()
                .getLatestBlockHeaderTimestamp()),
            this.khcApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderNumber(),
            this.khcApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderHash());
    ...
    try {
      ...
      synchronized (khcApp.getDbManager()) {
        ...
        if (slot == 0) {
          logger.info("Not time yet,now:{},headBlockTime:{},headBlockNumber:{},headBlockId:{}",
              new DateTime(now),
              new DateTime(
                  this.khcApp.getDbManager().getDynamicPropertiesStore()
                      .getLatestBlockHeaderTimestamp()),
              this.khcApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderNumber(),
              this.khcApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderHash());
        ...
        long timeout = Math
            .min(Parameter.ChainConstant.BLOCK_PRODUCED_INTERVAL * blockProducedTimeOut / 100 + 500,
                Parameter.ChainConstant.BLOCK_PRODUCED_INTERVAL);
        if (DateTime.now().getMillis() - now > timeout) {
          logger.warn("Task timeout ( > {}ms)，startTime:{},endTime:{}", timeout, new DateTime(now),
              DateTime.now());
          khcApp.getDbManager().eraseBlock();
  ...
  private void broadcastBlock(BlockCapsule block) {
    try {
      khcNetService.broadcast(new BlockMessage(block.getData()));
    } catch (Exception ex) {
  ...
  private BlockCapsule generateBlock(long when, ByteString witnessAddress,
    ...
    return khcApp.getDbManager().generateBlock(this.localWitnessStateMap.get(witnessAddress), when,
  ...
  public void checkDupWitness(BlockCapsule block) {
    ...
    if (System.currentTimeMillis() - block.getTimeStamp() > Parameter.ChainConstant.BLOCK_PRODUCED_INTERVAL) {
      return;
    }
  ...
  public void init() {
    ...
    WitnessCapsule witnessCapsule = this.khcApp.getDbManager().getWitnessStore()
