...
import io.fortest.protos.Contract.VoteAssetContract;
...
public class Util {
  ...
  public static byte[] generateContractAddress(Transaction kht, byte[] ownerAddress) {
    // get tx hash
    byte[] txRawDataHash = Sha256Hash.of(kht.getRawData().toByteArray()).getBytes();
      ...
      try {
        ...
        switch (contract.getType()) {
          ...
          case UnLockedBalanceContract:
            Contract.UnlockedBalanceContract unLockedBalanceContract = contractParameter
                    .unpack(Contract.UnlockedBalanceContract.class);
            contractJson = JSONObject
                    .parseObject(JsonFormat.printToString(unLockedBalanceContract, selfType));
            break;
      ...
      try {
        ...
        switch (contractType) {
          ...
          case "UnfreezeBalanceContract":
            ...
            break;
          case "UnLockedBalanceContract":
            Contract.UnlockedBalanceContract.Builder unlockedBalanceContract = Contract.UnlockedBalanceContract
                    .newBuilder();
            JsonFormat.merge(parameter.getJSONObject(VALUE).toJSONString(),
                    unlockedBalanceContract, selfType);
            any = Any.pack(unlockedBalanceContract.build());
            break;
          case "WithdrawBalanceContract":
