package io.fortest.core.services;
...
import io.fortest.api.DatabaseGrpc.DatabaseImplBase;
import io.fortest.api.GrpcAPI;
import io.fortest.api.GrpcAPI.AccountNetMessage;
import io.fortest.api.GrpcAPI.AccountPaginated;
import io.fortest.api.GrpcAPI.AccountResourceMessage;
import io.fortest.api.GrpcAPI.Address;
import io.fortest.api.GrpcAPI.AddressPrKeyPairMessage;
import io.fortest.api.GrpcAPI.AssetIssueList;
import io.fortest.api.GrpcAPI.BlockExtention;
import io.fortest.api.GrpcAPI.BlockLimit;
import io.fortest.api.GrpcAPI.BlockList;
import io.fortest.api.GrpcAPI.BlockListExtention;
import io.fortest.api.GrpcAPI.BlockReference;
import io.fortest.api.GrpcAPI.BytesMessage;
import io.fortest.api.GrpcAPI.DelegatedResourceList;
import io.fortest.api.GrpcAPI.DelegatedResourceMessage;
import io.fortest.api.GrpcAPI.EasyTransferAssetByPrivateMessage;
...
import io.fortest.api.GrpcAPI.EmptyMessage;
import io.fortest.api.GrpcAPI.ExchangeList;
import io.fortest.api.GrpcAPI.Node;
import io.fortest.api.GrpcAPI.NodeList;
import io.fortest.api.GrpcAPI.NumberMessage;
import io.fortest.api.GrpcAPI.PaginatedMessage;
import io.fortest.api.GrpcAPI.ProposalList;
import io.fortest.api.GrpcAPI.Return;
import io.fortest.api.GrpcAPI.Return.response_code;
import io.fortest.api.GrpcAPI.TransactionApprovedList;
import io.fortest.api.GrpcAPI.TransactionExtention;
import io.fortest.api.GrpcAPI.TransactionList;
import io.fortest.api.GrpcAPI.TransactionListExtention;
import io.fortest.api.GrpcAPI.TransactionSignWeight;
import io.fortest.api.GrpcAPI.WitnessList;
import io.fortest.api.WalletExtensionGrpc;
import io.fortest.api.WalletGrpc.WalletImplBase;
import io.fortest.api.WalletSolidityGrpc.WalletSolidityImplBase;
...
import io.fortest.core.WalletSolidity;
...
import io.fortest.protos.Contract.AssetIssueContract;
...
import io.fortest.protos.Protocol.DynamicProperties;
import io.fortest.protos.Protocol.Exchange;
import io.fortest.protos.Protocol.NodeInfo;
...
public class RpcApiService implements Service {
  ...
  private TransactionExtention transaction2Extention(Transaction transaction) {
    ...
    khtExtBuilder.setTransaction(transaction);
    khtExtBuilder.setTxid(Sha256Hash.of(transaction.getRawData().toByteArray()).getByteString());
    retBuilder.setResult(true).setCode(response_code.SUCCESS);
    khtExtBuilder.setResult(retBuilder);
    return khtExtBuilder.build();
  }
  ...
  public class WalletApi extends WalletImplBase {
    ...
    private void createTransactionExtention(Message request, ContractType contractType,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule kht = createTransactionCapsule(request, contractType);
        khtExtBuilder.setTransaction(kht.getInstance());
        khtExtBuilder.setTxid(kht.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (ContractValidateException e) {
      ...
      khtExtBuilder.setResult(retBuilder);
      responseObserver.onNext(khtExtBuilder.build());
      responseObserver.onCompleted();
    }
    ...
    public void getTransactionSign2(TransactionSign req,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule kht = wallet.getTransactionSign(req);
        khtExtBuilder.setTransaction(kht.getInstance());
        khtExtBuilder.setTxid(kht.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (Exception e) {
    ...
    public void addSign(TransactionSign req,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule kht = wallet.addSign(req);
        khtExtBuilder.setTransaction(kht.getInstance());
        khtExtBuilder.setTxid(kht.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
      } catch (Exception e) {
    ...
    private void checkVoteWitnessAccount(VoteWitnessContract req) {
      ...
      Preconditions.checkArgument(account.getMPower(dbManager) < votesCount,
              "khc power[" + account.getMPower(dbManager) + "] <  VotesCount[" + votesCount + "]");
    ...
    public void listNodes(EmptyMessage request, StreamObserver<NodeList> responseObserver) {
      ...
      nodeHandlerMap.entrySet().stream()
              .forEach(v -> {
                io.fortest.common.overlay.discover.node.Node node = v.getValue().getNode();
    ...
    private void callContract(Contract.TriggerSmartContract request,
      ...
      Return.Builder retBuilder = Return.newBuilder();
      try {
        TransactionCapsule khtCap = createTransactionCapsule(request,
                ContractType.TriggerSmartContract);
        Transaction kht;
        if (isConstant) {
          kht = wallet.triggerConstantContract(request, khtCap, khtExtBuilder, retBuilder);
        } else {
          kht = wallet.triggerContract(request, khtCap, khtExtBuilder, retBuilder);
        }
        khtExtBuilder.setTransaction(kht);
        khtExtBuilder.setTxid(khtCap.getTransactionId().getByteString());
        retBuilder.setResult(true).setCode(response_code.SUCCESS);
        khtExtBuilder.setResult(retBuilder);
