...
import io.fortest.protos.Contract.AssetIssueContract.FrozenSupply;
...
//      assetIssueCapsule.setOrder(order);
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (dbManager.getDynamicPropertiesStore().getAllowSameTokenName() != 0) {
      ...
      if (name.equals("kht")) {
        throw new ContractValidateException("assetName can't be kht");
      }
    }
    ...
    if (assetIssueContract.getTokenNum() <= 0) {
      throw new ContractValidateException("fortestNum must greater than 0!");
    }
