...
import io.fortest.protos.Protocol.Account.Builder;
...
import io.fortest.protos.Protocol.Vote;
...
public class AccountCapsule implements ProtoCapsule<Account>, Comparable<AccountCapsule> {
  ...
  //mp: M_Power
  public long getMPower(Manager dbManager) {
    long tp = 0;
    ...
    tp += account.getAcquiredDelegatedFrozenBalanceForBandwidth();
    tp += account.getAccountResource().getAcquiredDelegatedFrozenBalanceForEnergy();

    //others frozen for me
//    byte[] ownerAddress = account.getAddress().toByteArray();
//    DelegatedResourceAccountIndexCapsule accountIndexCapsule =
//            dbManager.getDelegatedResourceAccountIndexStore().get(account.getAddress().toByteArray());
//    if (accountIndexCapsule != null) {
//      List<ByteString> fromAccountsList = accountIndexCapsule.getFromAccountsList();
//      for (ByteString from : fromAccountsList) {
...
//                .createDbKey(fromAddress, ownerAddress);
//        DelegatedResourceCapsule delegatedResourceCapsule = dbManager.getDelegatedResourceStore()
//                .get(dbKey);
//        if (delegatedResourceCapsule != null) {
//          builder.addDelegatedResource(delegatedResourceCapsule.getInstance());
//          GrpcAPI.DelegatedResourceList delegatedResourceList = builder.build();
//          List<Protocol.DelegatedResource>  delegatedResources = delegatedResourceList.getDelegatedResourceList();
//          for (Protocol.DelegatedResource r : delegatedResources) {
//            Long frozenBalanceForBandwidth = r.getFrozenBalanceForBandwidth();
//            Long frozenBalanceForEnergy = r.getFrozenBalanceForEnergy();
//            if (null != frozenBalanceForBandwidth){
//              tp += frozenBalanceForBandwidth;
//            }
//            if (null != frozenBalanceForEnergy){
//              tp += frozenBalanceForEnergy;
//            }
//          }
//        }
//      }
...
//    tp += account.getAccountResource().getDelegatedFrozenBalanceForEnergy();
  ...
  public int getLockedCount(){
    return getInstance().getLockedCount();
  }
  public List<Frozen> getFrozenList() {
    return getInstance().getFrozenList();
  }

  public List<Account.Locked> getLockedList() {
    return getInstance().getLockedList();
  }
  ...
  public long getLockedBalance() {
    List<Account.Locked> lockedList = getLockedList();
    final long[] lockedBalance = {0};
    lockedList.forEach(locked -> lockedBalance[0] = Long.sum(lockedBalance[0],
  ...
  public void setLocked(long frozenBalance, long expireTime) {
    Account.Locked newLocked = Account.Locked.newBuilder()
            .setLockedBalance(frozenBalance)
            .setExpireTime(expireTime)
            .build();

    long lockedCount = getLockedCount();
    if (lockedCount == 0) {
      setInstance(getInstance().toBuilder()
              .addLocked(newLocked)
              .build());
  ...
  public void setStatus(Protocol.AccountStatus status) {
    this.account = this.account.toBuilder().setStatus(status).build();
  }
  ...
  public Protocol.AccountStatus getAccountStatus(){
