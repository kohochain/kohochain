...
import io.fortest.core.capsule.utils.MerkleTree;
...
import io.fortest.protos.Protocol.BlockHeader;
...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public BlockCapsule(long timestamp, ByteString parentHash, long number,
    ...
    // block
    Block.Builder blockBuild = Block.newBuilder();
    transactionList.forEach(kht -> blockBuild.addTransactions(kht));
    this.block = blockBuild.setBlockHeader(blockHeader).build();
    initTxs();
  }
  ...
  public void addTransaction(TransactionCapsule pendingkht) {
    this.block = this.block.toBuilder().addTransactions(pendingkht.getInstance()).build();
    getTransactions().add(pendingkht);
  }
  ...
  private void initTxs() {
    transactions = this.block.getTransactionsList().stream()
        .map(kht -> new TransactionCapsule(kht))
