  ...
  public class DisConnectScore extends Score<NodeStatistics> {
    ...
    int calculate(int baseScore) {
      if (t.wasDisconnected()) {
        if (t.getkhcLastLocalDisconnectReason() == null
            && t.getkhcLastRemoteDisconnectReason() == null) {
          // means connection was dropped without reporting any reason - bad
          baseScore *= 0.8;
        } else if (t.getkhcLastLocalDisconnectReason() != ReasonCode.REQUESTED) {
          // the disconnect was not initiated by discover mode
          if (t.getkhcLastRemoteDisconnectReason() == ReasonCode.TOO_MANY_PEERS
              || t.getkhcLastLocalDisconnectReason() == ReasonCode.TOO_MANY_PEERS
              || t.getkhcLastRemoteDisconnectReason() == ReasonCode.TOO_MANY_PEERS_WITH_SAME_IP
              || t.getkhcLastLocalDisconnectReason() == ReasonCode.TOO_MANY_PEERS_WITH_SAME_IP
              || t.getkhcLastRemoteDisconnectReason() == ReasonCode.DUPLICATE_PEER
              || t.getkhcLastLocalDisconnectReason() == ReasonCode.DUPLICATE_PEER
              || t.getkhcLastRemoteDisconnectReason() == ReasonCode.TIME_OUT
              || t.getkhcLastLocalDisconnectReason() == ReasonCode.TIME_OUT
              || t.getkhcLastRemoteDisconnectReason() == ReasonCode.PING_TIMEOUT
              || t.getkhcLastLocalDisconnectReason() == ReasonCode.PING_TIMEOUT
              || t.getkhcLastRemoteDisconnectReason() == ReasonCode.CONNECT_FAIL
              || t.getkhcLastLocalDisconnectReason() == ReasonCode.CONNECT_FAIL) {
            // The peer is popular, but we were unlucky
            baseScore *= 0.9;
          } else if (t.getkhcLastLocalDisconnectReason() == ReasonCode.RESET) {
            baseScore *= 0.95;
          } else if (t.getkhcLastRemoteDisconnectReason() != ReasonCode.REQUESTED) {
