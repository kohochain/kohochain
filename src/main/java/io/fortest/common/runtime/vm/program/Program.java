...
import static io.fortest.common.runtime.utils.MUtil.transferAllToken;
import static io.fortest.common.utils.BIUtil.toBI;
...
import io.fortest.common.utils.BIUtil;
import io.fortest.common.utils.ByteUtil;
import io.fortest.common.utils.FastByteComparisons;
...
import io.fortest.core.exception.ForTestException;
...
import io.fortest.common.runtime.vm.MessageCall;
import io.fortest.common.runtime.vm.OpCode;
import io.fortest.common.runtime.vm.PrecompiledContracts;
import io.fortest.common.runtime.vm.VM;
import io.fortest.common.runtime.vm.VMConstant;
import io.fortest.common.runtime.vm.program.listener.CompositeProgramListener;
import io.fortest.common.runtime.vm.program.listener.ProgramListenerAware;
import io.fortest.common.runtime.vm.program.listener.ProgramStorageChangeListener;
import io.fortest.common.runtime.vm.trace.ProgramTrace;
import io.fortest.common.runtime.vm.trace.ProgramTraceListener;
...
import io.fortest.protos.Protocol.SmartContract.Builder;
...
public class Program {
  ...
  /**
   * @param transferAddress the address send kht to.
   * @param value the kht value transferred in the internaltransaction
   */
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = convertTokhcAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = convertTokhcAddress(obtainerAddress.getLast20Bytes());
      ...
      try {
        ...
        if (VMConfig.allowTvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all kht failed in suicide: %s", e.getMessage());
        }
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
      boolean isCreate2) {
    byte[] senderAddress = convertTokhcAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (VMConfig.allowTvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setkhtHash(ByteString.copyFrom(rootTransactionId));
        }
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(convertTokhcAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = convertTokhcAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = convertTokhcAddress(getContractAddress().getLast20Bytes());
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        try {
          ...
          if (VMConfig.allowTvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), "refund energy from message call");
            throw new TransferException("transfer kht failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addBalance(senderAddress, -endowment);
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(convertTokhcAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(convertTokhcAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] khcAddr = convertTokhcAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(khcAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(khcAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(khcAddr, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState().getBalance(convertTokhcAddress(address.getLast20Bytes()));
  ...
  public DataWord storageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(convertTokhcAddress(getContractAddress().getLast20Bytes()), key.clone());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState().getTokenBalance(convertTokhcAddress(address.getLast20Bytes()),
  ...
  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
            .getContract(MUtil.convertTokhcAddress(address.getLast20Bytes()));
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    byte[] senderAddress = convertTokhcAddress(this.getCallerAddress().getLast20Bytes());
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(convertTokhcAddress(msg.getType().callIsDelegate()
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(ForTestException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(ForTestException e) {
      return new PrecompiledContractException(e.getMessage());
    }
